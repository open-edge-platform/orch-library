...
func (c *Ack[I]) Do(controller *Controller[I]) {
	controller.Log.Debugw("Reconciliation complete", dazl.String("Request.ID", fmt.Sprintf("%s", c.request.ID)))
}
...
func (r *Requeue[I]) Do(controller *Controller[I]) {
	controller.Log.Debugw("Requeueing request", dazl.String("Request.ID", fmt.Sprintf("%s", r.request.ID)))
...
func (f *Fail[I]) Do(controller *Controller[I]) {
	controller.Log.Warnw("Reconciliation failed",
		dazl.String("Request.ID", fmt.Sprintf("%s", f.request.ID)), dazl.Error(f.Error))
}
...
func (r *Retry[I]) Do(controller *Controller[I]) {
	controller.Log.Debugw("Reconciliation failed. Retrying...",
		dazl.String("Request.ID", fmt.Sprintf("%s", r.request.ID)), dazl.Error(r.Error))
	go controller.enqueue(r.request)
}
